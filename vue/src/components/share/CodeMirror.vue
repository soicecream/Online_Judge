<template>
  <div style="margin: 0px 0px 15px 0px;font-size: 14px;position: relative">
            ...
            <span v-else class="hidden-sm-and-down">
              <el-tooltip :content="$t('m.Exit_Focus_Mode')" placement="bottom">
                <el-button @click="switchFocusMode(false)" size="small">
                  <svg focusable="false" viewBox="0 0 1024 1024" fill="currentColor" width="0.95em" height="0.95em"
                       aria-hidden="true">
                    <path
    ...
    <el-drawer :visible.sync="openTestCaseDrawer" style="position: absolute;" :modal="false" size="40%"
      ...
      <el-tabs v-model="testJudgeActiveTab" type="border-card" style="height: 100%;" @tab-click="handleClick">
          ...
          <div v-loading="testJudgeLoding">
            <template v-if="testJudgeRes.status == -10">
              <div class="tj-res-tab mt-10">
                <el-alert :title="$t('m.Non_Test_Judge_Tips')" type="info" center :closable="false" show-icon>
                </el-alert>
              </div>
            </template>
        ...
        <el-tab-pane>
          ...
          <template v-if="!isAuthenticated">
            <div class="tj-res-tab mt-10">
              <el-alert :title="$t('m.Please_login_first')" type="warning" center :closable="false" show-icon/>
            </div>
          </template>
        </el-tab-pane>
      </el-tabs>
    ...
    this.editor.on("inputRead", (instance, changeObj) => {
      if (/\w|\./g.test(changeObj.text[0]) && changeObj.origin !== "complete") {
        instance.showHint({
          ...
          completeSingle: false, range: 1000, // 附近多少行代码匹配
        });
      }
    });
    this.$nextTick(() => {
  ...
  methods: {
    ...
    submitTestJudge() {
      if (!this.isAuthenticated) {
        this.$message.warning("Please_login_first");
      ...
      if (this.value.trim() === "") {
        this.$message.error("Code_can_not_be_empty")
        // myMessage.error(this.$i18n.t("m.Code_can_not_be_empty"));
        return;
      }

      if (this.value.length > 65535) {
        this.$message.error("Code_Length_can_not_exceed_65535")
    ...
    checkTestJudgeStatus() {
      ...
      const checkStatus = () => {
        // api.getTestJudgeResult(this.testJudgeKey).then((res) => {
        ...
        //             if (example.input == this.testJudgeRes.userInput && example.output == this.testJudgeRes.expectedOutput) {
